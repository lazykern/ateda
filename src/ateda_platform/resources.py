...
class SparkOperatorResource(ConfigurableResource):
    ...
    def setup_for_execution(self, context: InitResourceContext) -> None:
        ...
        self._k8s_custom_objects_api = kubernetes.client.CustomObjectsApi(
            self._k8s_client
        )
    ...
    def _load_k8s_config(self):
        ...
        self._logger.info(
            f"Loading Kubernetes configuration (using config file: {self.kubeconfig_path or 'in-cluster or default'})"
        )
        try:
            if self.kubeconfig_path:
                ...
                try:
                    ...
                    self._logger.info(
                        "In-cluster config failed, trying default kubeconfig."
                    )
    ...
    def _monitor_spark_application(
        ...
        if not self._k8s_custom_objects_api or not self._logger:
            raise Failure(
                "Kubernetes API client or logger not initialized. Call setup_for_execution first."
            )
        ...
        while True:
            ...
            try:
                ...
                if app_state == "COMPLETED":
                    logger.info(
                        f"SparkApplication '{app_name}' completed successfully."
                    )
                    # Optionally delete successful app
                    try:
                        if self.cleanup_on_success:
                            logger.info(
                                f"Cleaning up successful SparkApplication '{app_name}'..."
                            )
                            ...
                            logger.info(
                                f"Successfully deleted SparkApplication '{app_name}'."
                            )
                    except kubernetes.client.ApiException as del_e:
                        logger.warning(
                            f"Failed to delete completed Spark app {app_name}: {del_e}"
                        )
                    return True

                elif app_state in ["FAILED", "SUBMISSION_FAILED", "UNKNOWN"]:
                    logger.error(
                        f"SparkApplication '{app_name}' failed with state {app_state}."
                    )
                    if error_message:
                    ...
                    try:
                        if self.cleanup_on_failure:
                            logger.info(
                                f"Cleaning up failed SparkApplication '{app_name}'..."
                            )
                            api.delete_namespaced_custom_object(
                                group=SPARK_OP_GROUP,
                                version=SPARK_OP_VERSION,
                            ...
                            logger.info(
                                f"Successfully deleted failed SparkApplication '{app_name}'."
                            )
                    except kubernetes.client.ApiException as del_e:
                        logger.warning(
                            f"Failed to delete failed Spark app {app_name}: {del_e}"
                        )
                    raise Failure(
                        f"SparkApplication '{app_name}' failed. State: {app_state}. Error: {error_message or 'None'}"
                    )
                ...
                if e.status == 404:
                    if time.time() - start_time < 60:
                        logger.warning(
                            f"SparkApplication '{app_name}' not found yet, likely still creating. Retrying..."
                        )
                    else:
                        logger.error(
                            f"SparkApplication '{app_name}' not found after 60 seconds. Assuming creation failed or deleted externally."
                        )
                        raise Failure(
                            f"SparkApplication '{app_name}' disappeared or failed to create."
                        )
                else:
                    logger.error(
                        f"API Error monitoring SparkApplication '{app_name}': {e.reason} (status: {e.status})"
                    )
                    raise Failure(
                        f"API Error monitoring SparkApplication {app_name}"
                    ) from e
            except Exception as e:
                logger.error(
                    f"Unexpected error monitoring SparkApplication '{app_name}': {e}"
                )
                raise Failure(
                    f"Unexpected error monitoring SparkApplication {app_name}"
                ) from e
    ...
    def run_spark_application(self, spark_application: SparkApplication) -> bool:
        ...
        if not self._k8s_custom_objects_api or not self._logger:
            raise Failure(
                "Resource not initialized. Ensure setup_for_execution was called."
            )
        ...
        logger.info(
            f"Submitting SparkApplication '{app_name}' to namespace '{namespace}'"
        )
        ...
        try:
            ...
            logger.error(
                f"Failed to submit SparkApplication: {e.reason} (status: {e.status})"
            )
            try:
            ...
            raise Failure(
                f"Unexpected error submitting SparkApplication {app_name}"
            ) from e
        ...
        try:
            ...
            logger.error(
                f"SparkApplication '{app_name}' monitoring resulted in Failure: {e}"
            )
            raise e  # Re-raise the Failure
        except Exception as e:
            job_failed = True  # Mark as failed for potential cleanup
            logger.error(
                f"Unexpected error during SparkApplication '{app_name}' monitoring phase: {e}"
            )
            raise Failure(
                f"Unexpected error monitoring SparkApplication {app_name}"
            ) from e
            ...
            if job_failed and self.cleanup_on_failure:
                try:
                    ...
                    logger.info(
                        f"Cleaning up SparkApplication '{app_name}' after unexpected monitoring failure..."
                    )
                    api.delete_namespaced_custom_object(
                        group=SPARK_OP_GROUP,
                        version=SPARK_OP_VERSION,
                    ...
                    if del_e.status == 404:
                        logger.info(
                            f"SparkApplication '{app_name}' already gone, no cleanup needed after monitoring failure."
                        )
                    else:
                        logger.warning(
                            f"Failed to delete Spark app {app_name} after monitoring failure: {del_e}"
                        )
                except Exception as cleanup_e:
                    logger.warning(
                        f"Unexpected error during final cleanup of Spark app {app_name}: {cleanup_e}"
                    )
